...
struct MenuBarPopoverView: View {
    ...
    private final class ViewModel: ObservableObject {
        ...
        private static func fetchMetricsOffMain(account: Account) async -> AccountMetrics {
            await Task.detached(priority: .utility) {
                ...
                async let claudeAIQuotaHistory = ClaudeAIQuotaHistoryStore.shared.history(for: account.id, limit: 8)
                        ...
                        if filteredAccounts.isEmpty {
                            ...
                            ForEach(filteredAccounts) { account in
                                Button(displayName(for: account)) {
                                    guard let index = activeAccounts.firstIndex(where: { $0.id == account.id }) else { return }
    ...
    private func claudeAIStatsSection(account: Account, metrics: AccountMetrics?) -> some View {
        return VStack(alignment: .leading, spacing: 4) {
            if let status = metrics?.claudeAIStatus {
                ...
                statRow("Reset time", value: status.resetAt?.formatted(date: .abbreviated, time: .shortened) ?? "Unknown")
                statRow("Reset in", value: resetCountdownText(for: status))
                statRow("Session", value: claudeAISessionText(for: status))
                if config.claudeAI.notifyOnLowMessages && status.messagesRemaining <= config.claudeAI.lowMessagesThreshold {
                ...
                if let lastSuccessfulSyncAt = status.lastSuccessfulSyncAt {
                    statRow("Last healthy sync", value: lastSuccessfulSyncAt.formatted(date: .abbreviated, time: .shortened))
                }
    ...
    private func accountFetchLabel(account: Account, metrics: AccountMetrics?) -> some View {
        Group {
            if let lastFetch = metrics?.lastSuccess ?? metrics?.latestSnapshot?.timestamp {
    ...
    private func selectPreviousAccount() {
        guard let selected = AccountSelectionService.selectPrevious(in: activeAccounts),
              let index = activeAccounts.firstIndex(where: { $0.id == selected.id }) else { return }
        selectedAccountIndex = index
    }
